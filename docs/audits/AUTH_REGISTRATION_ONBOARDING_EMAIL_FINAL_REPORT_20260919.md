...
**BLOCKED — `BLOCKED: no GitHub credential in the execution environment.`** `git push` fails with `fatal: could not read Username for 'https://github.com'`; the PAT recorded on file is truncated (only a leading prefix was recorded) and cannot be used. Remedy (one line, then push and PR creation are immediate):
