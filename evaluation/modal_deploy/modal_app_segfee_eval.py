# ==============================================================================
# EVALUATION-ONLY COPY of services/vton-worker/modal_app_segfee.py (Phase 0.5)
# Changes vs production file (EXACTLY 2 identifiers):
#   1. modal.App name: confit-vton-worker-segfee -> confit-vton-worker-segfee-eval
#   2. Modal Secret name: confit-worker-admin-token -> confit-vton-eval-admin-token
# Engine code, image build, weights volume, GPU class, endpoints: IDENTICAL to production.
# This is a separate deployment for baseline measurement only. It does not touch
# the production worker, production secret, or any production endpoint.
# ==============================================================================
# ==============================================================================
...
app = modal.App("confit-vton-worker-segfee-eval")
...
_ENGINE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "services", "vton-worker", "engine"))
...
@app.cls(
    ...
    secrets=[modal.Secret.from_name("confit-vton-eval-admin-token")],
...
class FashnInferenceService:
    ...
    def process(self, payload: VTONJobRequest, x_vton_admin: str | None = Header(None, alias="X-VTON-Admin")) -> dict:
        ...
        # secret `confit-vton-eval-admin-token` (injected into the container env as
