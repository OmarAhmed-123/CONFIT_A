...
describe("same-tick render after state change (stale-closure regression)", () => {
  beforeEach(() => {
    ...
    mocks.getCapabilities.mockImplementation((ids: number[]) =>
      Promise.resolve({
        ...
        unsupported_slots: [],
        products: ids.map((id) => capabilityFor(id, "upper_inner")),
      }),
    );
  });
  ...
  it("photo upload renders with the FRESH photo even when state is still null", async () => {
    ...
    await act(async () => {
      result.current.setUploadedUserImage(
        "data:image/jpeg;base64,WHATEVER_WAS_BEFORE",
      );
      result.current.runTryOn({
        userImageUrl: "data:image/jpeg;base64,JUST_UPLOADED",
      });
    });
    await waitFor(() =>
  ...
  it("no-override render still uses committed state (camera-scan path unaffected)", async () => {
    ...
    expect(result.current.uploadedUserImage).toBe(
      "data:image/jpeg;base64,COMMITTED",
    );
