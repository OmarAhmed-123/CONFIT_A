...
import numpy as np
import torch
from ._parser_compat import CATEGORY_TO_BODY_COVERAGE
...
class TryOnPipeline:
    ...
    # NOTE (commercial fork): upstream's FashnHumanParser (a SegFormer-derived,
    # NVIDIA non-commercial work) is intentionally REMOVED from the runtime
    # path. This fork runs exclusively segmentation-free: see __call__ below,
    # which enforces segmentation_free=True and flat-lay garments, so the
    # segmentation arrays are never produced and never consumed.
    ...
    def __call__(
        ...
        """
        # ── Commercial-fork enforcement ─────────────────────────────────────
        # This fork has no human parser. It is valid ONLY when the person is not
        # masked (segmentation-free) AND the garment is a flat-lay product shot
        # (so the garment image is also returned unchanged). Any request that
        # would need the parser's segmentation output is rejected loudly rather
        # than silently degraded. Do NOT ship a masked/model-garment path here.
        if not segmentation_free:
            raise ValueError(
                "fashn-vton-segfee is a segmentation-free-only fork; the "
                "non-commercial upstream human parser is removed. "
                "segmentation_free=False is unsupported."
            )
        if garment_photo_type != "flat-lay":
            raise ValueError(
                "fashn-vton-segfee requires garment_photo_type='flat-lay'; a "
                "'model' (worn) garment needs segmentation, which is removed."
            )
        # ────────────────────────────────────────────────────────────────────
        ...
        # Human parsing — REMOVED (commercial fork).
        #
        # Upstream calls self.hp_model.predict(...) here on every inference and
        # then feeds the segmentation into the clothing-agnostic / garment
        # image steps. Upstream ALSO reports that segmentation-free is the
        # recommended mode for body preservation and unconstrained garment
        # volume, and in that mode the segmentation is discarded anyway
        # (the *_masking steps short-circuit on disable_masking=True).
        #
        # This fork is segmentation-free ONLY, so the parser's segmentation
        # output is genuinely absent from the execution graph. We still pass
        # None through the (now no-op, mask-early-return) helpers to keep the
        # call signature faithful; the guards in _parser_compat / agnostic.py
        # fail loudly if a masked (non-segfee) path is ever requested.
