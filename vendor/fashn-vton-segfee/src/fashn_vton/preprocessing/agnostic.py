...
import numpy as np
from .._parser_compat import BODY_COVERAGE_TO_LABELS, IDENTITY_LABELS, LABELS_TO_IDS

from ..utils import setup_logger
from .masks import asymmetric_dilate_mask, create_bounded_mask, create_contour_following_mask, dilate_mask

# Re-export constants (clean-room; see _parser_compat.py) for convenience
...
def create_garment_image(
    ...
    if not disable_masking:
        if seg_pred is None:
            raise ValueError(
                "seg_pred is None but garment masking was requested. The "
                "segmentation-free fork cannot mask without a parser; use "
                "garment_photo_type='flat-lay' (disable_masking=True)."
            )
...
def create_clothing_agnostic_image(
    ...
    # Commercial fork: a masked path requires a real segmentation array, which
    # the (removed) parser used to supply. If seg_pred is None we cannot mask,
    # so fail loudly rather than produce a wrong image.
    if seg_pred is None:
        raise ValueError(
            "seg_pred is None but masking was requested. The segmentation-free "
            "fork cannot mask without a parser; use disable_masking=True "
            "(segmentation-free / flat-lay)."
        )
